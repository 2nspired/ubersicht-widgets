...
| Widget changes (edits to `index.jsx`/layouts) don't appear, but numbers still update | Übersicht's file-watcher can die silently, leaving a stale compiled UI running fresh data. Fully quit Übersicht and relaunch — note `killall Übersicht` can fail silently on the umlaut (Unicode mismatch); use `pkill -f "bersicht.app"` or quit from the menu-bar icon. If it persists, also clear `~/Library/Caches/tracesOf.Uebersicht`. |
